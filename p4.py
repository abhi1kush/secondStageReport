...
def assign_denning(G,assign_str, PC):  # applying dennig's model on assignments
    ...
    if left in G:
        for pc in PC:
            output.append(make_lub_string(pc)+ " " + const.lt + " " + make_glb_string(label[left])) 
    else:
...
def set_clear_denning(G, called_by_fun, fun_global, expr_str, PC):
    ...
    if attr == "set" or attr == "clear":
        ...
        #print "set clear-> ",PC
        for pc in PC:
            output.append( make_lub_string(pc)+ " " + const.lt + " " + make_glb_string(label[var_name])) #label[left]
    elif attr == "wait":
