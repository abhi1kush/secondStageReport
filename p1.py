...
def assign_denning(assign_str, PC):  # applying dennig's model on assignments
    ...
    #updating PC
    for pc in PC:
            output.append(make_lub_string(pc+rvalue)+ " " + const.lt + " " + make_glb_string([left])) 
    line += 1
    return PC[:]
...
def set_clear_denning( called_by_fun, fun_global, expr_str, PC):
    ...
    if attr == "set" or attr == "clear":
        ...
        #print "set clear-> ",PC
        for pc in PC:
            output.append( make_lub_string(pc+[var_name])+ " " + const.lt + " " + make_glb_string([var_name])) #label[left]
    #elif attr == "wait":
...
def continuous_parse( funPC, called_by_fun, fun_global, data, PC):
    ...
    while i < length - 1:
        ...
        if parse_keyword(i, data) == "Assign":
            ...
            i = tmp[1]
            rest = tmp[2]
            if_denning(called_by_fun, fun_global, if_str, rest, PC[:])
            memo = {}
            PC = copy._deepcopy_list(PC_reset,memo)
        elif parse_keyword(i, data) == "While":
