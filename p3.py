...
def assign_denning(G,assign_str, PC):  # applying dennig's model on assignments
    ...
    if left in G:
        for pc in PC:
            output.append(make_lub_string(convertToLabelList(pc)+l)+ " " + const.lt + " " + make_lub_string(label[left])) 
    else:
        # update dynamic label, for all x (MNG(BR) union {pc}) [lambda(x)=l+lambda(x)+lambda(pc)]
        if left not in label:
            label[left] = []

        label[left] =  list(set(label[left] + make_lubPC(PC[:]+[l])))
        if left in label[left]:
            label[left].remove(left)
...
def set_clear_denning(G, called_by_fun, fun_global, expr_str, PC):
    ...
    if attr == "set" or attr == "clear":
        ...
        #print "set clear-> ",PC
        for pc in PC:
            output.append( make_lub_string(pc+[var_name])+ " " + const.lt + " " + make_lub_string(label[var_name])) #label[left]
    elif attr == "wait":
    ...
    while i < length - 1:
        ...
        if parse_keyword(i, data) == "Assign":
            ...
            if "value=Name(id='threading'" in assign_str:
                continue
            assign_denning(G,assign_str, PC[:])
        elif parse_keyword(i, data) == "If":
            debugPrint("If stmt:")
            ls = duplicateRemoval(PC)
            memo = {}
            PC_reset = copy._deepcopy_list(ls, memo)
            global ww1
            ww1 += 1
            i += 2
            tmp = parse_if(i, data)
            ...
            i = tmp[1]
            rest = tmp[2]
            if_denning(G,called_by_fun, fun_global, if_str, rest, PC[:])
            memo = {}
            PC = copy._deepcopy_list(PC_reset, memo)
        elif parse_keyword(i, data) == "While":
            ...
            while( it <= iteration ):
                ...
                #print "-> PC:",PC
                if lastPC == PC:
                    #print "Saturation point of loop!",PC,lastPC
                    break
                it += 1
            memo = {}
            PC = copy._deepcopy_list(PC_reset, memo)
